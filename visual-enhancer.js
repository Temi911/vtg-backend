(()=>{
  const PRODUCTS=[
    {name:'Premium Vehicles',tag:'Cars • SUVs • Commercial vehicles',img:'https://images.unsplash.com/photo-1542282088-72c9c27ed0cd?q=85&w=1600&auto=format&fit=crop'},
  ...
  const wait=()=>{const f=document.getElementById('vtgFrame');if(!f)return setTimeout(wait,100);const apply=()=>{try{const d=f.contentDocument,w=f.contentWindow;if(!d)return;
    // Fix icon timing: frontend-v3 calls lucide before its CDN script has loaded.
    const ensureIcons=()=>{try{if(w.lucide&&typeof w.lucide.createIcons==='function'){w.lucide.createIcons({attrs:{'stroke-width':1.9}})}}catch(e){console.warn('VTG icon refresh',e)}};
    ensureIcons();setTimeout(ensureIcons,250);setTimeout(ensureIcons,1000);w.addEventListener('load',ensureIcons);
    const mo=new w.MutationObserver(()=>ensureIcons());mo.observe(d.body,{childList:true,subtree:true});
    ...
    if(hero){
      ...
      PRODUCTS.forEach((p,i)=>{const im=d.createElement('img');im.src=p.img;im.alt=p.name+' — '+p.tag;im.className=i===0?'active':'';im.loading=i<2?'eager':'lazy';im.onerror=()=>{im.style.background='linear-gradient(135deg,#123b57,#0e969f)';im.removeAttribute('src')};carousel.appendChild(im)});
      const overlay=d.createElement('div');overlay.className='vtgSupplierOverlay';carousel.appendChild(overlay);
      const dots=d.createElement('div');dots.className='vtgSupplierDots';PRODUCTS.forEach((_,i)=>{const s=d.createElement('span');if(i===0)s.className='active';dots.appendChild(s)});carousel.appendChild(dots);
      ...
      if(old)old.remove();hero.prepend(carousel);
      const title=hero.querySelector('.copy h3'),para=hero.querySelector('.copy p');let i=0;
      const update=()=>{const p=PRODUCTS[i];carousel.querySelectorAll('img').forEach((x,n)=>x.classList.toggle('active',n===i));dots.querySelectorAll('span').forEach((x,n)=>x.classList.toggle('active',n===i));if(title)title.textContent=p.name+' — supplier presence.';if(para)para.textContent=p.tag+'. Upload products, specifications, company media, adverts and videos so buyers can discover and enquire.'};
      update();setInterval(()=>{i=(i+1)%PRODUCTS.length;update()},4200);
    ...
    if(hero&&!d.querySelector('.vtgProductStrip')){
      const strip=d.createElement('div');strip.className='vtgProductStrip';PRODUCTS.slice(0,8).forEach(p=>{const c=d.createElement('div');c.className='vtgProductChip';c.innerHTML='<img src="'+p.img+'" alt="'+p.name+'"><div><b>'+p.name+'</b><small>'+p.tag+'</small></div>';c.querySelector('img').onerror=()=>c.querySelector('img').remove();strip.appendChild(c)});hero.parentElement.appendChild(strip)
    }
    // Keep all newly inserted Lucide icons mature and consistent.
    ensureIcons();
  }catch(e){console.warn('VTG visual enhancer failed',e)}};f.addEventListener('load',apply,{once:true});try{if(f.contentDocument?.readyState==='complete')apply()}catch(e){}};
