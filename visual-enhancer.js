(() => {
  ...
  function apply() {
    try {
      ...
      .productHero .copy{z-index:4}.tradeCard img,.newsItem img,.newsLarge img,.mini img{background:#e8f1f3}
      ...
      if (hero) {
        ...
        PRODUCTS.forEach((p, i) => {
          const im = d.createElement('img'); im.src = p.img; im.alt = p.name + ' — ' + p.tag; im.className = i === 0 ? 'active' : ''; im.loading = i < 2 ? 'eager' : 'lazy';
          im.onerror = () => { im.style.background = 'linear-gradient(135deg,#123b57,#0e969f)'; im.removeAttribute('src') }; carousel.appendChild(im);
        });
        ...
        update(); setInterval(() => { i = (i + 1) % PRODUCTS.length; update() }, 4200); dots.querySelectorAll('span').forEach((s, n) => s.onclick = () => { i = n; update() });
      }
      if (hero && !d.querySelector('.vtgProductStrip')) {
        const strip = d.createElement('div'); strip.className = 'vtgProductStrip'; PRODUCTS.slice(0, 8).forEach(p => { const c = d.createElement('div'); c.className = 'vtgProductChip'; c.innerHTML = '<img loading="lazy" src="' + p.img + '" alt="' + p.name + '"><div><b>' + p.name + '</b><small>' + p.tag + '</small></div>'; c.querySelector('img').onerror = () => c.querySelector('img').remove(); strip.appendChild(c) }); hero.parentElement.appendChild(strip);
      }
      ensureIcons();
