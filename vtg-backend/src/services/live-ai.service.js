...
function buildInstructions(country, role) {
  ...
  return [
    ...
    'Use Google Search grounding for any fact that can change: current prices, exchange rates, tariffs, customs procedures, government fees, regulations, shipping conditions, market news, company information, product availability, public officials, or recent developments.',
...
async function publicChat({ message, history = [], country, role }) {
  ...
  const body = {
    ...
    contents,
    tools: [{ google_search: {} }],
    generationConfig: { maxOutputTokens: 1600, temperature: 0.25 },
  };
